...
pub async fn fetch_remote_installed_marketplaces(
    ...
    let global = async {
        let scope = RemotePluginScope::Global;
        let installed_plugins = fetch_installed_plugins_for_scope(config, auth, scope).await?;
        Ok::<_, RemotePluginCatalogError>(installed_plugins)
    };
    let workspace = async {
        let scope = RemotePluginScope::Workspace;
        let installed_plugins = fetch_installed_plugins_for_scope(config, auth, scope).await?;
    ...
    let (global_installed_plugins, workspace_installed_plugins) =
        tokio::try_join!(global, workspace)?;
    let mut marketplaces = Vec::new();
    ...
    let mut listed_plugins = Vec::new();
    let mut private_plugins = Vec::new();
    let mut unlisted_plugins = Vec::new();
    for plugin in workspace_installed_plugins {
        match workspace_plugin_discoverability(&plugin.plugin)? {
            RemotePluginShareDiscoverability::Listed => listed_plugins.push(plugin),
            RemotePluginShareDiscoverability::Private => private_plugins.push(plugin),
            RemotePluginShareDiscoverability::Unlisted => unlisted_plugins.push(plugin),
        }
    }

    for (name, display_name, plugins) in [
        (
            REMOTE_WORKSPACE_MARKETPLACE_NAME,
